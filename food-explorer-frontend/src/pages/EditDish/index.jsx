...
export function EditDish() {
    ...
    async function handleUpdateDish() {
        if (!image) toast.error("Erro: Você não carregou a nova imagem do prato!");
        if (!title) toast.error("Erro: Você não informou o nome do prato!");
        if (ingredients.length < 1) toast.error("Erro: Adicione pelo menos um ingrediente!");
        if (newIngredient) toast.error("Erro: Você deixou um ingrediente no campo para adicionar, mas não clicou em adicionar. Clique no sinal de + para adicionar!");
        if (!category) toast.error("Erro: Você não selecionou a categoria do prato!");
        if (!price) toast.error("Erro: Você não informou o preço do prato!")
        if (!description) toast.error("Erro: Você não informou uma descrição para o prato!");
        ...
        await api
            ...
            .then(toast.error("Prato atualizado com sucesso!"), navigate("/"))
            .catch((error) => {
                if (error.response) {
